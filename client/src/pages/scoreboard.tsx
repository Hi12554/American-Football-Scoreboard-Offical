...
export default function Scoreboard() {
  ...
  const setQuarter = (quarter: 1 | 2 | 3 | 4 | "HT" | "OT") => {
    // Set clock to 13 minutes (780 seconds) when halftime is selected
    if (quarter === "HT") {
      setLocalTime(780);
      updateStateMutation.mutate({ 
        quarter,
  ...
  const clearFlag = useCallback(() => {
    updateStateMutation.mutate({ activeFlag: null });
  }, []);
  ...
  const finalizeGame = useCallback(() => {
    ...
    const parseRecord = (record: string) => {
      // Support both W-L and W-L-T formats
      const matchWithTies = record.match(/^(\d+)-(\d+)-(\d+)$/);
      if (matchWithTies) {
        return { 
          wins: parseInt(matchWithTies[1]), 
          losses: parseInt(matchWithTies[2]),
          ties: parseInt(matchWithTies[3])
        };
      }
      
      const matchNoTies = record.match(/^(\d+)-(\d+)$/);
      if (matchNoTies) {
        return { 
          wins: parseInt(matchNoTies[1]), 
          losses: parseInt(matchNoTies[2]),
          ties: 0
        };
      }
    ...
    if (gameState.isFinal) {
      // Unfinal the game - revert records
      const homeRecord = parseRecord(gameState.homeTeam.record);
      const awayRecord = parseRecord(gameState.awayTeam.record);

      let newHomeRecord = gameState.homeTeam.record;
      let newAwayRecord = gameState.awayTeam.record;

      if (gameState.homeTeam.score > gameState.awayTeam.score) {
        // Home team won - subtract the win/loss
        newHomeRecord = formatRecord(homeRecord.wins - 1, homeRecord.losses, homeRecord.ties);
        newAwayRecord = formatRecord(awayRecord.wins, awayRecord.losses - 1, awayRecord.ties);
      } else if (gameState.awayTeam.score > gameState.homeTeam.score) {
        // Away team won - subtract the win/loss
        newHomeRecord = formatRecord(homeRecord.wins, homeRecord.losses - 1, homeRecord.ties);
        newAwayRecord = formatRecord(awayRecord.wins - 1, awayRecord.losses, awayRecord.ties);
      } else {
        // Tie - subtract the tie
        newHomeRecord = formatRecord(homeRecord.wins, homeRecord.losses, homeRecord.ties - 1);
        newAwayRecord = formatRecord(awayRecord.wins, awayRecord.losses, awayRecord.ties - 1);
      }
      ...
      updateStateMutation.mutate({
        ...
        homeTeam: {
          ...gameState.homeTeam,
          record: newHomeRecord,
        },
        awayTeam: {
          ...gameState.awayTeam,
          record: newAwayRecord,
        },
      });
    } else {
      // Finalize the game
      ...
      if (gameState.homeTeam.score > gameState.awayTeam.score) {
        ...
        // Tie - increment tie count for both teams
        newHomeRecord = formatRecord(homeRecord.wins, homeRecord.losses, homeRecord.ties + 1);
        newAwayRecord = formatRecord(awayRecord.wins, awayRecord.losses, awayRecord.ties + 1);
      }

      // Update game state with final status and new records
      updateStateMutation.mutate({
        isFinal: true,
        isClockRunning: false,
        homeTeam: {
  ...
  const copyOverlayLink = useCallback(() => {
